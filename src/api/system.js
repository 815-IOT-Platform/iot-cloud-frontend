...
// post岗位Api
export function getSysPostList (parameter) {
  return axios({
    url: api.post + '/list',
    method: 'get',
    params: parameter
...
export function saveSysPost (parameter) {
  return axios({
    url: api.post + (parameter.postId > 0 ? '/update' : '/save'),
    method: 'post',
    data: parameter,
    headers: {
      'Content-Type': 'application/json;charset=UTF-8'
...
export function delSysPost (parameter) {
  return axios({
    url: api.post + '/remove',
    method: 'post',
    params: parameter
